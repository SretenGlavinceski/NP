package Midterm1.midterm2020.ex2;
...
class HospitalSystem<T extends Patient & Comparable<T>> {
    ...
    ArrayList<T> patients;

    HospitalSystem(ArrayList<T> patients) {
    ...
    public void printResults() {
        ...
        PatientService<T> countAllWithGenderAndAgeLessThanSixty = (patients, filter) ->
                patients.stream().filter(patient -> patient.getAge() < 60 && patient.getGender().equals(filter))
                .collect(Collectors.toCollection(ArrayList::new));
        ...
        PatientService<T> findAllWithNameSorted = (patients, filter) ->
                patients.stream().filter(patient -> patient.getName().equals(filter))
