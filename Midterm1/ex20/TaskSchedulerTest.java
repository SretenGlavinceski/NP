...
//    @Override
//    default int compareTo(Task o) { // with compareTo method using Comparable, bellow is with using Comparator.comparing(Task::getOrder)
...
class Schedulers {
    public static <T extends Task> TaskScheduler<T> getOrdered() {
        return array -> Arrays.stream(array)
                .sorted(Comparator.comparing(Task::getOrder)).collect(Collectors.toList());
    }

    public static <T extends Task> TaskScheduler<T> getFiltered(int order) {
        return array -> Arrays.stream(array)
                .filter(i -> i.getOrder() < order).collect(Collectors.toList());
    }
}