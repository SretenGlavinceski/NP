...
class Order {
    ...
    public void addItem(Item item, int count) throws OrderLockedException, ItemOutOfStockException {
        ...
        if (items.stream().anyMatch(i -> i.getItem().getType().equals(item.getType()))) {
            items.stream().filter(i -> i.getItem().getType().equals(item.getType())).forEach(i -> i.setCount(count));
            return;
        }
