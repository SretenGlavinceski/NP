...
class Scheduler <T> {
    ...
    Timestamp<T> next() {
        return timestamps.stream()
                .filter(i -> i.getTime().isAfter(LocalDateTime.now()))
                .min(Timestamp::compareTo).orElse(null);
    }

    Timestamp<T> last() {
        return timestamps.stream()
                .filter(i -> i.getTime().isBefore(LocalDateTime.now()))
                .max(Timestamp::compareTo).orElse(null);
    }

    List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        return timestamps.stream()
                .filter(i -> i.getTime().isAfter(begin) && i.getTime().isBefore(end))
